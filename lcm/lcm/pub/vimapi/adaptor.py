...
def create_flavor(vim_cache, res_cache, data, flavor, do_notify, res_type):
    ...
    flavor_extra_specs = ""
    memory_page_size_str = virtual_compute["virtual_memory"]["vdu_memory_requirements"]["memoryPageSize"]
    if memory_page_size_str:
        memory_page_size = int(memory_page_size_str.replace('MB', '').strip())
