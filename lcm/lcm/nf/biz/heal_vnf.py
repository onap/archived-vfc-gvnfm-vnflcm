...
class HealVnf(InstantiateVnf, Thread):
    def __init__(self, data, nf_inst_id, job_id):
        InstantiateVnf.__init__(self, data, nf_inst_id, job_id)
    ...
    def heal_pre(self):
        if not (self.action == HEAL_ACTION_TYPE.START or self.action == HEAL_ACTION_TYPE.RESTART):
        ...
        if not (len(self.vm_id) and len(self.vdu_id) and len(self.vm_name)):
