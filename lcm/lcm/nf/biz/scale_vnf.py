...
from lcm.nf.const import GRANT_TYPE, CHANGE_TYPE, SUB_OPERATION_TASK
...
from lcm.pub.exceptions import NFLCMException, NFLCMExceptionInterrupt
...
class ScaleVnf(Thread):
    ...
    def do_notify_create_vim_res(self, res_type, ret):
        ...
        self.check_op_is_cancel()

    def check_op_is_cancel(self):
        lcm_op_occ_obj = self.lcm_op_occ.get()
        if lcm_op_occ_obj.sub_operation == SUB_OPERATION_TASK.CANCEL:
            raise NFLCMExceptionInterrupt('VNF(%s) scale_out is canceled.' % self.nf_inst_id)
