...
def check_auth_types(auth_types):
    for auth_type in auth_types:
        if auth_type not in const.AUTH_TYPES:
            raise serializers.ValidationError(
                "The auth_type %s is not a " +
                "valid one. The allowed values " +
                "are: %s" % (auth_type, ",".join(const.AUTH_TYPES)))
    return auth_types
